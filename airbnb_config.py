...
class ABConfig():
    ...
    def __init__(self):
        ...
        try:
            ...
            # look for username.config on both Windows (USERNAME) and Linux
            # (USER)
            if os.name == "nt":
            ...
            # database
            try:
                self.DB_HOST = config["DATABASE"]["db_host"] if (
                ...
                logging.error("Incomplete database information in " +
                              config_file + ": cannot continue.")
                sys.exit()
            # network
            try:
                self.HTTP_PROXY_LIST = config["NETWORK"]["proxy_list"].split(
                    ",")
                self.HTTP_PROXY_LIST = [x.strip()
                                        for x in self.HTTP_PROXY_LIST]
            except Exception as ex:
                logging.warning("No proxy_list in " +
                                config_file + ": not using proxies")
                self.HTTP_PROXY_LIST = []
            try:
                self.USER_AGENT_LIST = config["NETWORK"]["user_agent_list"].split(
                    ",,")
                self.USER_AGENT_LIST = [x.strip()
                                        for x in self.USER_AGENT_LIST]
                self.USER_AGENT_LIST = [x.strip('"')
                                        for x in self.USER_AGENT_LIST]
            except Exception:
            ...
            # survey
            self.FILL_MAX_ROOM_COUNT = int(
                config["SURVEY"]["fill_max_room_count"])
            self.ROOM_ID_UPPER_BOUND = int(
                config["SURVEY"]["room_id_upper_bound"])
            ...
            self.RE_INIT_SLEEP_TIME = float(
                config["SURVEY"]["re_init_sleep_time"])
