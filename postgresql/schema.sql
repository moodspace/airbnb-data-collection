...
CREATE TABLE city (city_id integer NOT NULL,
...
CREATE TABLE search_area (search_area_id serial NOT NULL,
                                                name character varying(255) DEFAULT 'UNKNOWN'::character varying, abbreviation character varying(10),
                                                                                                                                         bb_n_lat double precision, bb_e_lng double precision, bb_s_lat double precision, bb_w_lng double precision, CONSTRAINT search_area_pkey PRIMARY KEY (search_area_id)) WITH (OIDS=FALSE);
...
CREATE TABLE room (room_id integer NOT NULL,
                                   host_id integer, room_type character varying(255),
                                                                        country character varying(255),
                                                                                          city character varying(255),
                                                                                                         neighborhood character varying(255),
                                                                                                                                address character varying(1023),
                                                                                                                                                  reviews integer, overall_satisfaction double precision, accommodates integer, bedrooms numeric(5,2),
                                                                                                                                                                                                                                         bathrooms numeric(5,2),
                                                                                                                                                                                                                                                   price double precision, deleted integer, minstay integer, last_modified TIMESTAMP WITHOUT TIME ZONE DEFAULT now(),
...
CREATE TABLE neighborhood (neighborhood_id integer NOT NULL,
                                                   name character varying(255),
                                                                  search_area_id integer);
...
CREATE TABLE survey_progress_log (survey_id integer NOT NULL,
                                                    room_type character varying(255) NOT NULL,
                                                                                     neighborhood_id integer, page_number integer NOT NULL,
...
CREATE TABLE zipcode (zipcode character varying(10) NOT NULL,
                                                    search_area_id integer);
...
ALTER COLUMN neighborhood_id
SET DEFAULT nextval('neighborhood_neighborhood_id_seq'::regclass);
...
ALTER COLUMN search_area_id
SET DEFAULT nextval('search_area_search_area_id_seq'::regclass);
...
SET DEFAULT nextval('survey_survey_id_seq'::regclass);
...
SET DEFAULT nextval('survey_search_page_page_id_seq'::regclass);
...
ALTER TABLE ONLY neighborhood ADD CONSTRAINT neighborhood_pkey PRIMARY KEY (neighborhood_id);
...
ALTER TABLE ONLY room ADD CONSTRAINT room_pkey PRIMARY KEY (room_id,
                                                            survey_id);
...
ALTER TABLE ONLY survey_progress_log ADD CONSTRAINT survey_search_page_pkey PRIMARY KEY (page_id);
...
ALTER TABLE ONLY zipcode ADD CONSTRAINT z PRIMARY KEY (zipcode);
...
CREATE OR REPLACE FUNCTION trg_location() RETURNS TRIGGER AS $BODY$
BEGIN
...
UPDATE OF latitude,
          longitude ON room
FOR EACH ROW EXECUTE PROCEDURE trg_location();
...
ALTER TABLE ONLY zipcode ADD CONSTRAINT zipcode_search_area_id_fkey
FOREIGN KEY (search_area_id) REFERENCES search_area(search_area_id);
...
--
 REVOKE ALL ON SEQUENCE city_city_id_seq
FROM PUBLIC;
...
--
 REVOKE ALL ON TABLE search_area
FROM PUBLIC;
...
--
 REVOKE ALL ON TABLE neighborhood
FROM PUBLIC;
...
--
 REVOKE ALL ON SEQUENCE neighborhood_neighborhood_id_seq
FROM PUBLIC;
...
--
 REVOKE ALL ON SEQUENCE search_area_search_area_id_seq
FROM PUBLIC;
...
--
 REVOKE ALL ON TABLE survey_progress_log
FROM PUBLIC;
...
--
 REVOKE ALL ON SEQUENCE survey_search_page_page_id_seq
FROM PUBLIC;
...
--
 REVOKE ALL ON SEQUENCE survey_survey_id_seq
FROM PUBLIC;
